...
function applyFilters() {
  const kw = (state.search || '').trim().toLowerCase(); const chosen = state.selectedTags; let list = state.posts.filter(p => chosen.size === 0 || (p.tags || []).some(t => chosen.has(t)));
  if (kw) {
...
function renderPagination(pages) {
  const nav = document.getElementById('pagination'); if (!nav) return; nav.innerHTML = ''; if (pages <= 1) return; const prev = document.createElement('button'); prev.className = 'page-btn'; prev.textContent = '上一页'; prev.disabled = state.page <= 1; prev.onclick = () => { if (state.page > 1) { state.page--; renderAll() } };
...
function renderTimeline(list) {
  const timeline = document.getElementById('timeline'); if (!timeline) return; const grouped = groupByYear(list); const frag = document.createDocumentFragment();
  grouped.forEach(g => {
