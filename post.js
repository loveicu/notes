...
async function loadMarkdownWithFallback(slug){const tried=[];const add=(s)=>{if(s&&!tried.includes(s))tried.push(s)};const base=slug||'';add(base);if(/-\d+$/.test(base))add(base.replace(/-\d+$/,''));const u2h=base.replace(/_/g,'-');if(u2h!==base)add(u2h);const h2u=base.replace(/-/g,'_');if(h2u!==base)add(h2u);const lc=base.toLowerCase();if(lc!==base)add(lc);for(const s of tried){const url=`./posts/${encodeURIComponent(s)}.md`;const text=await tryFetch(url);if(text!=null)return{text,used:s,url}}return{error:true,tried:tried.map(s=>`./posts/${s}.md`)}}

async function loadPost(){const yearEl=document.getElementById('year');if(yearEl)yearEl.textContent=new Date().getFullYear();initTheme();initPaletteUI();
  ...
  try{const r=await loadMarkdownWithFallback(slug);if(r.error){throw new Error('未找到文章。已尝试：\n'+r.tried.join('\n'))}
